...
TaskView::TaskView(QWidget *parent)
    : QScrollArea(parent)
{
    TaskWatcherPanel = makeTaskPanel();
    taskStack.addWidget(TaskWatcherPanel);

    this->setWidget(&taskStack);
...
QSize TaskView::minimumSizeHint() const
{
    ...
    if (QLayout* layout = taskStack.currentWidget()->layout()) {
        spacing = 2 * layout->spacing();
    }

    ms.setWidth(taskStack.currentWidget()->minimumSizeHint().width() + spacing);
    return ms;
}
...
QSint::ActionPanel* TaskView::makeTaskPanel()
{
    auto taskPanel = new TaskPanel(this);
...
bool TaskView::showDialog(TaskDialog *dlg, App::Document* doc)
{
    ...
    outInfo.taskPanel = makeTaskPanel();
    if (dlg->buttonPosition() == TaskDialog::North) {
        outInfo.taskPanel->addWidget(outInfo.ActiveCtrl);
        for (const auto & it : cont){
            outInfo.taskPanel->addWidget(it);
        }
    }
    else {
        for (const auto & it : cont){
    ...
    outInfo.taskPanel->setScheme(QSint::ActionPanelScheme::defaultScheme());

    if (!dlg->needsFullSpace()) {
        outInfo.taskPanel->addStretch();
    }
...
void TaskView::removeDialog(std::vector<TaskInfo>::iterator infoIt)
{
    ...
    if (infoIt->ActiveCtrl) {
        infoIt->taskPanel->removeWidget(infoIt->ActiveCtrl);
    ...
    if (infoIt->ActiveDialog) {
        ...
        if (infoIt->ActiveDialog->property("taskview_accept_or_reject").isNull()) {
            ...
            for(const auto & it : cont){
                infoIt->taskPanel->removeWidget(it);
            }
    ...
    infoIt->taskPanel->removeStretch();

    taskStack.removeWidget(infoIt->taskPanel);
...
void TaskView::addTaskWatcher()
{
    ...
    for (TaskWatcher* tw : ActiveWatcher) {
        ...
        for (QWidget* w : cont) {
            TaskWatcherPanel->addWidget(w);
        }
    }

    if (!ActiveWatcher.empty()) {
        TaskWatcherPanel->addStretch();
    }
    updateWatcher();
    ...
    auto box = TaskWatcherPanel->findChild<QDialogButtonBox*>();
    if (box) {
        QEvent event(QEvent::StyleChange);
        QApplication::sendEvent(box, &event);
    }

    TaskWatcherPanel->setScheme(QSint::ActionPanelScheme::defaultScheme());
    taskStack.setCurrentIndex(0);
}
...
void TaskView::removeTaskWatcher()
{
    ...
    for (TaskWatcher* tw : ActiveWatcher) {
        ...
        for (QWidget* w : cont) {
            w->hide();
            TaskWatcherPanel->removeWidget(w);
        }
    }

    TaskWatcherPanel->removeStretch();
}
...
void TaskView::clearActionStyle()
{
    ...
    if (current) {
        ...
        current->taskPanel->setScheme(QSint::ActionPanelScheme::defaultScheme());
    }
}
